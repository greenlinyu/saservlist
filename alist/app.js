...
app.use('/', createProxyMiddleware({
  target: 'http://127.0.0.1:port',  // ✅ 改为实际监听端口
  changeOrigin: true,
  ws: true,
  onError: (err, req, res) => {
    res.writeHead(500, {
...
function keep_aria2_alive() {
  exec("pgrep -laf aria2c", function (err, stdout, stderr) {  //改成进程名，如：alist
    if (stdout.includes("aria2c --enable-rpc --rpc-listen-port")) {  //改成进程实际运行命令，如：alist server
      console.log("aria2 正在运行");
    } else {
      exec(
        "nohup aria2c --enable-rpc --rpc-listen-port=6800 > /dev/null 2>&1 &",  // 改成启动脚本或者启动命令，如：chmod +x start.sh && ./start.sh 或者 ./alist server
        function (err, stdout, stderr) {
          if (err) {
            console.log("保活-调起aria2-命令行执行错误:" + err);
          } else {
            console.log("保活-调起aria2-命令行执行成功!");
          }
        }
      );
    }
...
app.listen(port, () => console.log(`AList wrapper listening on port ${port}!`));
